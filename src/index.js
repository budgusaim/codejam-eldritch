...
const enemyChoosePanel = document.querySelector(".choose-enemy"),
  gameMods = document.querySelectorAll(".mode"),
  cardHolder = document.querySelector(".enemy-card"),
  gameModeChoose = document.querySelector(".game-mode"),
...
enemyChoosePanel.addEventListener("change", function () {
  const enemy = this.value;
  ancients.forEach((item) =>
    item.title === enemy ? changeEnemyCard(item) : 0
  );
});

const changeEnemyCard = (item) => {
  ancient = item;
  const url = item.src;
  const img = new Image();
  img.src = url;
  ...
  gameModeChoose.classList.add("_active");
  showGameOptions();
};
...
gameMods.forEach((element) =>
  element.addEventListener("click", (e) => {
    const gameModsArr = Array.from(gameMods),
      gameOptionChoosen = gameModsArr.filter((element) => element === e.target);
    if (gameOptionChoosen) {
      gameMods.forEach((element) =>
...
const hiddenGameOptions = () => {
  gameModeChoose.classList.remove("_active");
